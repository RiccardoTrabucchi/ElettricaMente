/* --- DATABASE PROGETTI (MODIFICATO CON PIÙ FOTO) --- */
const projectsData = [
    { 
        id: 1, 
...
(function type(){
    ...
    document.getElementById('typing-placeholder').textContent=letter;
...
function init(){ p=[]; for(let i=0;i<60;i++) p.push({x:Math.random()*w,y:Math.random()*h,vx:(Math.random()-.5),vy:(Math.random()-.5)}); }
function draw(){
    ...
    if(prj){
        ...
        if(prj.images && prj.images.length > 0) {
            ...
            prj.images.forEach((src, idx) => {
                ...
                // Usa l'helper anche per le thumb
                setImage(thumb, src);
                thumb.onclick = () => {
...
function closeModalFunction() {
    ...
    // Aspetta la fine della transizione CSS (300ms) prima di sbloccare lo scroll
    setTimeout(() => {
